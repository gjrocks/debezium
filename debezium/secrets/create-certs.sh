...
# Generate CA key
openssl req -new -x509 -keyout snakeoil-ca-1.key -out snakeoil-ca-1.crt -days 365 -subj '/CN=localhost/OU=TEST/O=local/L=pune/S=MH/C=IN' -passin pass:confluent -passout pass:confluent
...
openssl req -passin "pass:confluent" -passout "pass:confluent" -key kafkacat.client.key -new -out kafkacat.client.req -subj '/CN=localhost/OU=TEST/O=local/L=pune/S=MH/C=IN'
